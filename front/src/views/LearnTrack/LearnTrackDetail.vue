...
const CET4Progress = computed(
  () => (CET4prcatice.value / CET4total.value) * 100,
);
const CET6Progress = computed(
  () => (CET6prcatice.value / CET6total.value) * 100,
);
...
async function getDetails() {
  await learnTrack.getWordNum_Today(auth.user as string).then((response) => {
  ...
  await learnTrack.getWordNum_CET4().then((response) => {
    if (response.code === 200) {
      const data = JSON.parse(response.resultData);
  ...
  await learnTrack.getSpeakProgress(auth.user as string).then((response) => {
    if (response.code === 200) {
      oralList.value = response.resultData;
    }
  });

  await learnTrack.getEssayProgress(auth.user as string).then((response) => {
    if (response.code === 200) {
...
<template>
  <v-col>
    <v-row>
      <v-col cols="6">
        <v-card hover prepend-icon="mdi-spellcheck" title="单词练习进度">
          <v-divider class="border-opacity-75" thickness="2" color="primary" />
          <v-card-text> 今日练习单词数量：{{ wordNum }} </v-card-text>
          <v-card-text>
      ...
      <v-col cols="6">
        <v-card hover prepend-icon="mdi-book-open-variant" title="阅读练习进度">
          <v-divider class="border-opacity-75" thickness="2" color="primary" />
          <v-card-text> 今日已阅读文章数量：{{ passageNum }} </v-card-text>
          <v-card-text>
            今日阅读外刊列表：
    ...
    <v-row class="row-style">
      <v-col cols="6">
        <v-card hover prepend-icon="mdi-account-voice" title="口语话题练习进度">
          <v-divider class="border-opacity-75" thickness="2" color="primary" />
          <v-card-text>
      ...
      <v-col cols="6">
        <v-card hover prepend-icon="mdi-pencil" title="写作练习进度">
          <v-divider class="border-opacity-75" thickness="2" color="primary" />
          <v-card-text> 今日已进行写作练习次数：{{ writeNum }} </v-card-text>
          <v-card-text>
            今日作文练习列表：
            <v-list lines="one" style="height: 80px; overflow-y: auto">
